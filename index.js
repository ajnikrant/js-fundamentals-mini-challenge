...
let sameSameButDifferent = function(num, maybeNum){
  let result;
  if (num === maybeNum){
    result = console.log("same same");
  } else if (num == maybeNum){
    result = console.log("same same (but different)");
  } else if (num !== maybeNum){
    result = console.log("different");
  }
  return result;
}
...
let updateGrade = function(student, grade1){
  var newGrade = student["grade"] = grade1;
  return student;
}
...
// ***** Question 6 *****
// *** Uncomment the lines below to test
const printNameAndPhones = function(user){
...
const takeATicketNumber = function(line){
  line.push(line.length +1);
  var newLine = line.length;
  return `Welcome. You are ticket number ${newLine}`;
}
...
// let ticketNumberGeneratorFunc = function(newLine){
//   return `${newLine}`;
// }

const takeATicketNumberFunc = function ticketNumberGeneratorFunc(newLine){
  newLine.push(newLine.length +1);
  spotInLine = newLine.length;
  return `Welcome. You are ticket number ${spotInLine}`;
}
